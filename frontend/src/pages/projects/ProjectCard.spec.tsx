  ...
  epics: [
    ...
    },
    {
      ...
      comments: [
        {
          id: 12,
          content: "Parfait ! Cette solution répond exactement à notre besoin.",
...
describe("ProjectCard", () => {
    ...
    render(
      <BrowserRouter>
        <ProjectCard
          ...
          onProjectDeleted={deleteFn}
          onProjectUpdated={updateFn}
        />
      </BrowserRouter>
    );

    await screen.findByText(MOCK_PROJECT.name);
    await screen.findByText("Actif");
    const epicCount = await screen.findByTestId("epic-count");
    expect(epicCount).toHaveTextContent(String(MOCK_PROJECT.epics.length));
    await screen.findByText(MOCK_PROJECT.creator.username);
    const teamNames = await screen.findAllByTestId("team-name");
    expect(teamNames).toHaveLength(MOCK_PROJECT.teams.length);
    teamNames.forEach((teamName, index) => {
      expect(teamName).toHaveTextContent(MOCK_PROJECT.teams[index].name);
    });
    const editButton = await screen.findByTestId("view-project-button");
    fireEvent.click(editButton);
    expect(mockedUseNavigate).toHaveBeenCalledWith(
      `/projects/${MOCK_PROJECT.slug}`
    );
  });

  it("should render the modal project", async () => {
    const updateFn = vi.fn();
    ...
    const MOCK_USER: JwtPayload = {
      email: "toto@mail.com",
      username: "Toto",
      uuid: MOCK_PROJECT.creator.uuid,
      iat: 1234567890,
      exp: 1234567890,
    };
    ...
    const updateBtn = await screen.findByTestId("edit-button");
  ...
  it("should delete the project", async () => {
    ...
    expect(deleteProjectSpy).toHaveBeenCalledWith(MOCK_PROJECT.slug);
    await waitFor(() => {
      expect(deleteFn).toHaveBeenCalled();
    });
    await waitFor(() => {
      const modal = screen.queryByTestId("delete-confirm");
